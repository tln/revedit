...
import (
	"fmt"
	"github.com/tln/revedit/text/template"
...
// Execute applies a parsed template to the specified data object,
// writing the output to wr.
func (t *Template) TraceExecute(wr io.Writer, data interface{}) (*template.Trace, error) {
	if err := t.escape(); err != nil {
		return nil, err
	}
	tr, err := t.text.TraceExecute(wr, data)
	return tr, err
}

// ExecuteTemplate applies the template associated with t that has the given
// name to the specified data object and writes the output to wr.
func (t *Template) TraceExecuteTemplate(wr io.Writer, name string, data interface{}) (*template.Trace, error) {
	tmpl, err := t.lookupAndEscapeTemplate(name)
	if err != nil {
		return nil, err
	}
	tr, err := tmpl.text.TraceExecute(wr, data)
	return tr, err
}
