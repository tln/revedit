...
type Node interface {
	...
	Extent() int   // number of bytes of node in full original input
...
func (l *ListNode) Extent() (extent int) {
	for _, n := range l.Nodes {
		extent += n.Extent()
	}
	return
...
func newPipeline(pos Pos, line int, extent int, decl []*VariableNode) *PipeNode {
	return &PipeNode{NodeType: NodePipe, Pos: pos, Line: line, Decl: decl, Ext: Ext(extent)}
}

func (p *PipeNode) append(command *CommandNode) {
	p.Cmds = append(p.Cmds, command)
	p.Ext = Ext(int(p.Ext) + command.Extent())
}
...
func (p *PipeNode) CopyPipe() *PipeNode {
	...
	n := newPipeline(p.Pos, p.Line, p.Extent(), decl)
...
func (a *ActionNode) Extent() int {
	return a.Pipe.Extent()
}
...
func newCommand(pos Pos, ext int) *CommandNode {
	return &CommandNode{NodeType: NodeCommand, Pos: pos, Ext: Ext(ext)}
}
