...
// Package parse builds parse trees for templates as defined by github.com/tln/tracetemplate
...
func (t *Tree) pipeline(context string) (pipe *PipeNode) {
	...
	extent := int(t.peekNonSpace().pos) - int(pos) // !!! not right
	pipe = newPipeline(pos, t.lex.lineNumber(), extent, decl)
	for {
		switch token := t.nextNonSpace(); token.typ {
...
func (t *Tree) command() *CommandNode {
	cmd := newCommand(t.peekNonSpace().pos, 0)
	for {
	...
	cmd.Ext = Ext(int(t.peekNonSpace().pos) - int(cmd.Pos))
	return cmd
}
